...
**Wiki Pages** shows the requested wiki, project, page title/path, returned revision if supplied, and returned sections. Explicit section metadata is preserved; otherwise headings are extracted from structured Markdown content using `marked`, ignoring headings inside fenced code. A returned level-one heading can supply a missing page title. Heading-derived sections do not claim source-line offsets. Wiki search snippets, page listings, and linked subpages are not counted as pages read.
