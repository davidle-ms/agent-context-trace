...
Only allowed HTTPS Azure DevOps web links are offered. Query parameters are stripped except repository `path`/`version` and wiki `pagePath`; links are revalidated against the selected session. Parsing is limited to 5,000 distinct file/wiki calls, 20 text blocks per call, and 2 MiB characters per inspected block. At most 100 explicit sections or Markdown headings are retained; heading parsing is limited to 128 KiB characters of structured Markdown. The first usable response block is shown, not a concatenation of arbitrary blocks. These adapters have synthetic parser/native UI coverage; no direct file/page calls were present in the bounded real-history sample, so their full server-version compatibility remains unverified.
...
| `npm run test:unit` | Twenty tests covering repository read-line mapping and escaping, repository/wiki evidence, Markdown sections, preview limits and redaction, work-item reads/batches/comments, safe links, plus file coverage, history replay, scope, persistence, and attribution. |
