...
Move the pointer down the map to scroll the corresponding source line into view, centered where possible. A contrasting, outlined horizontal line spans the heatmap at the mouse's exact height, separate from the static range guides and viewport outline. Only mouse movement over the heatmap positions this line; clicks, keyboard navigation, and source-editor hovers cannot place or reposition it. Leaving the map hides it. Hovering does not move the cursor, change selections, edit source, or record reads. The readout updates to `Line {number} / {total lines}` as the pointer moves and shows the exact displayed count and revision caveat. It stays visible in a separate footer while the map scrolls. Hovering source text in the active file also updates the readout, without placing a heatmap hover line, scrolling the editor, or changing its selection. Source-text updates use VS Code's hover provider and respect its hover delay and enabled setting; they do not add another tooltip. Click the map to focus that line in the editor. With keyboard focus on the heatmap, use Up/Down, Page Up/Page Down, or Home/End to navigate, and Enter/Space to focus the line. Keyboard navigation retains its own target so delayed editor-hover updates cannot change the line being activated.
