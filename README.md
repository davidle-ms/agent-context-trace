...
The preview includes an existing Copilot chat picker, a read-only local-history adapter, built-in Explorer and editor-section decorations, a hover-to-navigate file heatmap, Azure DevOps **Work Items**, **Repository Files**, and **Wiki Pages** activity tabs, and a persistent color toggle. Explicit instrumented tracker sessions remain available as an optional fallback. This is not a complete observer of all Copilot activity.
...
| `npm run test:unit` | Twenty-one tests covering saved UTF-8 text attachments, repository read-line mapping and escaping, repository/wiki evidence, Markdown sections, preview limits and redaction, work-item reads/batches/comments, safe links, plus file coverage, history replay, scope, persistence, and attribution. |
| `npm run test:extension` | Native repository/wiki/work-item tabs, on-demand plain-text previews, provenance, filtering, keyboard tabs, session isolation and live history updates; existing heatmap, grouped picker, and restart persistence checks. |
