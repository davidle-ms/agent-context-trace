...
test('sessions serialize calls, reject stale generations, and restore metadata only', async () => {
    ...
    try {
        ...
        assert.equal(store.get(session.id)?.currentLap, 1);
        assert.deepEqual(store.get(session.id)?.events.map(item => item.lap), [1, 1]);
        assert.equal(await store.addLap(session.id), 2);
        await assert.rejects(store.record(session.id, 0, event(), () => false));
        await store.record(session.id, 1, event(2, 4), () => false);
        assert.equal(store.get(session.id)?.events.at(-1)?.lap, 2);
        assert.equal(await store.removeLap(session.id), 1);
        assert.equal(store.get(session.id)?.currentLap, 1);
        assert.ok(store.get(session.id)?.events.every(item => item.lap === 1), 'Removing a lap merges its evidence into the previous lap');
        await assert.rejects(store.removeLap(session.id), /Lap 1 cannot be removed/);
        await assert.rejects(store.record(session.id, 1, event(), () => false));
        await store.setState(session.id, 'paused');
        await store.setState(session.id, 'recording');
        await assert.rejects(store.record(session.id, 0, event(), () => false));
        await assert.rejects(store.record(session.id, 4, event(), () => true));
        ...
        assert.deepEqual(Object.keys(persisted.events[0]).sort(), [...Object.keys(event()), 'lap'].sort());
        const legacy = parseSession({ ...persisted, currentLap: undefined, events: persisted.events.map(({ lap: _lap, ...item }: ReadEvent) => item) });
        assert.equal(legacy.currentLap, 1);
        assert.ok(legacy.events.every(item => item.lap === 1));
