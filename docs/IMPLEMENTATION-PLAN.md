...
Editor sections: the existing eye toggle now controls automatic line shading as well as Explorer filename colors. Blue sections require a matching recorded snapshot fingerprint. Explicit saved-chat ranges without a fingerprint display as amber, unverified guides; `agentContextTrace.showUnverifiedHistoryRanges` disables those guides. Missing or invalid ranges never shade an entire file. Historical guides are suppressed in dirty documents and after edits observed during the selected session. Verified stale ranges disappear when the fingerprint changes. Visible/split-editor changes, session changes, new reads, and toggles refresh decorations; disposal clears them. Fifteen core/parser tests and native rendering checks cover these behaviors. This supersedes the earlier separate/opt-in editor-highlight design and the blanket no-unverified-range-display rule below.
