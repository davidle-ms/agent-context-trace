...
Code searches and pipeline logs (2026-09-17): Code Searches and Pipeline Logs complete the five Azure DevOps activity categories alongside the File Heatmap tab. Search calls preserve bounded query/scope/page metadata and returned match locations, labelled as snippets rather than full-file reads; explicit snippet bodies require a preview request. Log get_content calls show build/log IDs, requested range separately from explicit returned offsets, and bounded saved text. Log listings and searches never create local file coverage. Both use existing session refresh, filtering/pagination, plain-text rendering, and export redaction. Twenty-three unit tests and native UI/watcher/preview tests pass; real saved search-result and log-line-array shapes were also checked read-only with aggregate-only output. Earlier search/log deferrals below are superseded for these supported envelopes, not for arbitrary MCP servers or internal server reads.

Repository read-lines view (2026-09-17): returned repository entries offer a View read lines link opening a reusable script-free read-only webview panel. Explicit ranges are mapped to amber numbered source rows only when consistent with the returned line count; otherwise the preview uses neutral response-line numbers and labels source mapping unavailable. Withheld, missing, and truncated previews are disclosed. The panel contains a saved call snapshot, never current remote content, and closes on session changes. Twenty unit tests and native link/panel safety, range fallback, reuse, and cleanup checks pass. No manifest contribution or local file content changes are required.

Repository/wiki activity (2026-09-17): additional Repository Files and Wiki Pages tabs parse saved `repo_file/get_content` and `wiki/get_page` calls. Requested paths/versions are separated from returned paths/revisions; source ranges must be explicit. Wiki sections come from response metadata or headings parsed with `marked`, never search-result inference. Bounded optional previews are plain text, on-demand, memory-only, excluded from all exports; redacted exports omit external metadata entirely. The existing file heatmap and Work Items remain separate. Nineteen unit tests and native tab/preview/watcher tests pass. No direct file/page calls were present in the sampled real history; these response adapters are fixture-verified rather than live-server-certified. Earlier no-response-text statements below are superseded only for these opt-in repository/wiki previews.

Azure DevOps work-item activity (2026-09-17): Agent Read Coverage now has File Heatmap and Work Items tabs. Saved MCP `get`, `get_batch`, and `list_comments` calls produce separate metadata-only call/item entries with explicit returned/unavailable/failed/cancelled/pending outcomes, requested versus returned field names, titles/revisions when present, and comment IDs/page counts. No field bodies or comment text are retained, no MCP requests are replayed, and remote work items never become file coverage. The UI filters and pages entries, opens validated Azure DevOps links, and resets on session changes. Redacted exports omit all work-item activity. Eighteen unit tests and native UI/watcher/pagination tests pass on the installed Windows host; the observed single-item schema was also validated on real saved history using aggregate-only output. Batch/comment and alternate tool formats are fixture-tested, not broadly server-version-certified. This adds a narrowly supported external-context adapter to the earlier file-only scope below.

File heatmap (2026-09-17): Agent Read Coverage is now a `WebviewViewProvider`, retaining its native header commands and status without adding a duplicate file tree. It follows the active local workspace editor and consumes the same revision-filtered per-line ranges as editor decorations. Hovering the proportional map reveals the corresponding logical source line without changing selection; clicking or keyboard activation focuses it. A visible-range outline follows editor scrolling. The eye toggle hides all three color surfaces. No source text or chat content enters the nonce-restricted, network-disabled webview, and navigation is bounded to the current visible editor with stale-state rejection. Native tests verify canvas pixels, compact and expanded layouts, top/middle/bottom alignment in a 240-line file, cursor/source preservation, keyboard navigation, file switching, and stale-history clearing. This supersedes the controls-only/no-webview description below. VS Code owns Explorer view sizing; users can enlarge the map with its native divider.

Editor sections: the existing eye toggle controls automatic line shading as well as Explorer filename colors. Recorded reads use four amber background intensities with amber edge/ruler markers: 1, 2-3, 4-7, and 8+ reads. Amber represents frequency for all evidence types, not a warning or an unverified revision. Syntax colors are unchanged and there are no inline count labels; Explorer filenames use matching amber frequency shades. Increasing opacity makes more-read sections more prominent in light and dark themes. An interval-boundary sweep computes exact per-line counts and splits overlapping ranges; equal-count adjacent spans merge. Read IDs are deduplicated. Explorer filenames use distinct per-file calls in the selected session, while editor counts exclude known nonmatching revisions and missing line ranges. Exact counts and provenance remain in tooltips, with a concise legend in the coverage section. A matching tracker fingerprint is identified in the tooltip; history without a fingerprint says "Historical range; file may have changed." The historical opt-out, edit invalidation, and visible/split-editor updates are preserved. Sixteen core/parser tests and native rendering checks cover all four amber filename, vertical-marker, and background tiers, unchanged text colors, file totals, overlaps, duplicate refreshes, and session isolation. This supersedes the former gray/blue colors, labels, and earlier editor-highlight proposals below.
