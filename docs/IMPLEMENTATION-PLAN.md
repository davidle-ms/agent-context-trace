...
Editor sections: the existing eye toggle controls automatic line shading as well as Explorer filename colors. All recorded ranges share one blue decoration style. A matching tracker fingerprint is identified in the tooltip; saved-chat ranges without a fingerprint say "Historical range; file may have changed." The `agentContextTrace.showUnverifiedHistoryRanges` setting still disables those historical guides. Missing or invalid ranges never shade an entire file. Historical guides are suppressed in dirty documents and after edits observed during the selected session. Verified stale ranges disappear when the fingerprint changes. Visible/split-editor changes, session changes, new reads, and toggles refresh decorations; disposal clears them. Fifteen core/parser tests and native rendering checks cover these behaviors, including identical colors for both evidence types. This supersedes the former two-color design and the earlier separate/opt-in editor-highlight proposals below.

Implemented: repository-grouped Copilot chat picker, opt-in read-only local-history adapter, selected-file watcher, optional registered read tool and tracker sessions, Explorer colors and in-editor section shading, a controls-only coverage section, persisted color toggle, read details, and JSON export. Fifteen core/parser tests and native host checks pass on Windows VS Code 1.138.0, including rendered verified/unverified sections, edit invalidation, split editors, session changes, grouping, history refresh, Explorer colors, and persistence.
