...
Toggle visibility: Agent Read Coverage now contains one native checkbox labeled Toggle Read Colors with an On/Off indicator beneath the status message. It replaces the eye-only header action; checkbox clicks, label clicks, keyboard interaction, and the Command Palette share the same persisted preference. This control is not a file-tree entry and has no children. Native UI tests verify the label, both states, all interaction modes, and unchanged session data. This supersedes the header-toggle proposals below.
...
The original plan below remains the target design, not a claim that every task is complete. Current source is consolidated into core, extension, and views modules; tests use Node's test runner plus assertion-based host tests, not Mocha. Initial storage limits are 1,000 events and 4 MiB per session, with 100 retained session files. Only prepared successful reads are persisted. Interrupted-session ownership, dynamic workspace roots, global storage budgeting, and large-repository performance need further hardening. See [README.md](../README.md) for actual usage and limitations.
