...
Native trees support foreground colors, icons, and badges, not arbitrary full-row background fills. This plan interprets "color in the files" as coloring their tree representation without modifying source files. Keep the effect scoped to this section; built-in Explorer Git colors and editor tabs must remain unchanged.

Use display-only `agent-context-trace:` resource URIs for this tree's items and a decoration provider that handles only that scheme. Keep a validated mapping to real local files for explicit open commands; never try to open the display URI or register a content provider that exposes source. Include workspace-root identity in the mapping to avoid duplicate-name collisions. Validate this rendering path in the Phase 0 host probe; a colored `ThemeIcon` with a textual read indicator is the native fallback if the supported host cannot decorate the custom tree's labels.
...
| T15 | `src/views.ts`: `provideFileDecoration(uri, token)`, `refreshFileColors()`, `openRead(event)`, `updateDecorations(editor, session)` | Handle display-only URIs with non-propagating color/badge metadata; invalidate colors on toggle/session/deletion; revalidate navigation and suppress stale editor highlights. | T06, T13, T14 | VS Code file decoration and navigation APIs | Medium |
...
| T20 | `src/test/extension.test.ts` | Host tests for repository enumeration without false reads, file colors, toggle persistence, recording while hidden, session isolation, Explorer/tab non-interference, trust, and Windows paths; visually verify native themes. | T18, T19 | Official extension testing sample | High |
...
| File colors | Register `FileDecorationProvider` restricted to this view's display-URI scheme; set color, `R` badge, tooltip, and `propagate: false`. Return no decoration when disabled, neutral, or outside scope. Contribute light/dark/high-contrast theme defaults. |
...
| Color scope | Switch sessions, delete selected history, inspect built-in Explorer/tabs and duplicate filenames in multi-root workspaces. | No stale-session or cross-root colors; only the dedicated section is decorated. Built-in Explorer Git colors and tabs remain unchanged. |
...
- [ ] Verify the off preference survives reload and file colors do not alter built-in Explorer, Git decorations, tabs, or source content.
